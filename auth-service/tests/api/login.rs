...
async fn should_return_401_if_incorrect_credentials(test_case: Value) {
    ...
    let (mut app, _) = prepare_login(true, false).await;
...
pub async fn prepare_login(
    requires_2fa: bool,
    requires_mock_email_server: bool,
) -> (TestApp, Value) {
...
async fn should_return_200_if_valid_credentials_and_2fa_disabled() {
    ...
    let (mut app, login_body) = prepare_login(enable_efa, false).await;
...
async fn should_return_206_if_valid_credentials_and_2fa_enabled() {
    let enable_2fa = true;
    let (mut app, login_body) = prepare_login(enable_2fa, true).await;
